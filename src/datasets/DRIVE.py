...
class DRIVE(torch.utils.data.Dataset):
    ...
    def __init__(self, data_path=DATA_PATH, transform=None):
        ...
        self.images = sorted(
            glob.glob(os.path.join(data_path, "training/images/*.tif"))
        )
        self.vessel_masks = sorted(
            glob.glob(os.path.join(data_path, "training/1st_manual/*.gif"))
        )
        self.fov_masks = sorted(
            glob.glob(os.path.join(data_path, "training/mask/*.gif"))
        )
